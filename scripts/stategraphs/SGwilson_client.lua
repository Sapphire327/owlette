...
table.insert(states, State{
    ...
    onenter = function(inst, data)
        ...
        if data then
            ...
            if target and target:IsValid() and target.components and target.components.spawner then
                local spawner = target.components.spawner
                if spawner.child and spawner.child:IsValid() then
                    ...
                    if child and child:IsValid() and child.components.locomotor then
                        local angle = child:GetAngleToPoint(inst:GetPosition()) + 180
